...
must_haves:
  truths:
    ...
    - "Checkout API validates budget before creating Stripe session and returns error if config exceeds budget at minimum runs"
    - "Checkout API reduces runs per model when projected cost exceeds $7 budget"
    - "Webhook generates a share_token (nanoid) for each new report for public report links"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/app/api/checkout/route.ts"
      ...
      via: "Budget pre-calculation and enforcement before Stripe session creation"
...
<task type="auto">
  ...
  <action>
    ...
    **Budget pre-calculation and enforcement (BEFORE payment):**
    Import `estimateCost`, `optimizeRunsForBudget` from `@/lib/wizard/cost-estimator` and `getModelById` from `@/lib/config/models`.
    Resolve selected model IDs from the draft's config_data to ModelInfo objects via getModelById().
    Call `estimateCost({ selectedModels, runsPerModel: config_data.sampleCount, sampleCount: images.length })`.
    - If projected cost > API_BUDGET_CEILING ($7):
      - Call `optimizeRunsForBudget(selectedModels, images.length)` to get reduced runs per model.
      - If optimized runs < 1 (even 1 run per model exceeds budget):
        - Return 400 with `{ error: "budget_exceeded", message: "This configuration exceeds the budget even at minimum runs. Please reduce the number of models or images.", modelCount: selectedModels.length, imageCount: images.length }`.
      - Else: store the optimized `runsPerModel` in the checkout metadata (so the engine uses the reduced count). Log the reduction.
    - If within budget: proceed normally.
    The budget validation is strictly internal -- the user never sees cost numbers, only the error message if their config is too large.
    ...
    - Use createAdminClient to create a report directly (simulating post-payment):
      ...
      - Set status to 'paid'
      - Generate share_token: `nanoid(22)` (URL-safe, unique public identifier for the report)
      - Generate a fake stripe_session_id: `mock_${nanoid()}`
      - If budget optimization reduced runs, store optimized runsPerModel in config_snapshot
    ...
    - Create Stripe Checkout session using stripe.checkout.sessions.create:
      ...
      - metadata: { draft_id: draftId, user_id: user.id, optimized_runs_per_model: runsPerModel (if reduced) }
    ...
    **3. Stripe webhook handler (`src/app/api/webhooks/stripe/route.ts`):**
    ...
    - Extract draft_id, user_id, and optimized_runs_per_model from session.metadata
    ...
    - Create report record:
      ...
      - share_token: `nanoid(22)` (URL-safe public identifier; column already exists in reports table from 001_initial_schema.sql)
      - config_snapshot: { config_data, upload_data, schema_data, selected_models } from draft. If optimized_runs_per_model is present in metadata, override config_data.sampleCount with it in the snapshot.
  ...
  <verify>
    ...
    - POST `/api/checkout` with over-budget config returns reduced runsPerModel or 400 error
    - Webhook handler at `/api/webhooks/stripe` validates signatures and creates report records with share_token
  ...
  <done>
    Complete Stripe checkout flow works end-to-end: user sees confirmation screen with benchmark summary and $14.99 price, clicks Pay, checkout API validates budget (reducing runs or rejecting if too large), redirects to Stripe (or mock skips redirect), webhook creates report idempotently with share_token, user lands on processing page. Cancel page offers retry. Mock mode works without Stripe keys.
  </done>
</task>
